...
> **Version note:** The `json%` elaboration syntax lives in `Lean.Data.Json.Elab`.
> Antiquotation and object-key rules may evolve across toolchain versions; verify
> against the current source if behavior differs from what is documented here.

## Scope

Reference for constructing JSON values in Lean 4 using `json%` elaboration syntax,
`Json.mkObj`, and `ToJson` instances.
Useful for any Lean code that constructs JSON — scripts, plugins, metaprograms, tooling.

**Read when:** building JSON payloads, interpolating Lean values into JSON, deriving or
using `ToJson`, or debugging `json%` elaboration errors.

**Not part of the prove/autoprove default loop.** This is supplemental reference
material for projects that produce JSON output.
...
1. Import `Lean.Data.Json` (or at least `Lean.Data.Json.Elab` and
...
4. Keep object keys static (`ident` or string literal); switch to `Json.mkObj` for
   dynamic keys.
...
- Objects elaborate to `Lean.Json.mkObj [...]`. Because `mkObj` builds a `Std.TreeMap`,
  duplicate keys collapse (last value wins) and output order is map order, not insertion
  order.

- Object keys accept either `ident` or string literal.
  Keys that are Lean keywords must be quoted.
...
- Antiquotation uses `Lean.toJson`:
  ...
  - Top-level antiquotation works too: `json% $expr` elaborates to `toJson expr`. If
    `expr : Json`, this embeds it unchanged (`ToJson Json = id`).
...
`json%` does not support antiquotation in key position.
Build dynamic-key objects manually.
...
`Json.opt` emits nothing for `none` and a key-value pair for `some`, avoiding explicit
branching.
...
- **Fields reordered in `pretty`/`compress`:**
  ...
  - This is expected. Objects are stored in `TreeMap` order, not insertion order.
...
- **Key-related parse issues:**
  ...
  - Keys that are Lean keywords (e.g. `meta`, `where`, `import`) must be quoted as
    string literals.
...
- [metaprogramming-patterns](metaprogramming-patterns.md) — if building elaborators that
  produce JSON
