...
Follow this workflow for all skill acquisition tasks to ensure research depth and tool
accuracy.
...
- **Hunt the Raw Source**: Always locate the authoritative GitHub repository.
  Webfetch the **raw** `SKILL.md` file from `https://raw.githubusercontent.com/...` to
  bypass site-specific UI blockers or JS-rendering requirements.

- **Report Negatives**: If research is exhausted, report the gap using the Epistemic
  Integrity format:
...
- **Installation Locations**: Most agent CLIs (Smithery, LobeHub, Claude Code, [[codex/SKILL|Codex]],
  ...
  Always check if the skill already exists in the canonical path before manually
  copying.
...
- **Verify Canonical Path**: Install/copy all skill content strictly into
  `~/ai/opencode/skills/skill-name/`.

- **Manual Installation**: If CLI fails, manually download raw files and place them in
  the canonical path.

- **Strict Fabrication Ban**: NEVER write or fabricate skill content from memory.
  Content must be fetched, verified, and saved using the `write` tool.
...
When the CLI search doesn’t find a skill or you’re having difficulty locating it, use
these techniques:

### Direct URL Inspection

The LobeHub CLI search may not find skills by partial identifier.
Instead, **webfetch the skill page directly** using the URL from the user’s request:
...
The identifier format is typically `{author}-{repo}-{skill-name}`. After locating a
skill:
...
Smithery is the canonical tool for searching, inspecting, and installing MCP servers and
agent skills.
...
Available agent targets: claude-code, `cursor`, [[codex/SKILL|codex]], `windsurf`, opencode,
`github-copilot`, and 30+ others.
Run `npx -y @smithery/cli skill agents` to list them.

## LobeHub Marketplace

LobeHub Marketplace contains a wide variety of skills, including many for mathematical,
scientific, and research agent workflows.
Use the `@lobehub/market-cli` tool for discovery operations.
...
This creates credentials at `~/.lobehub-market/credentials.json`. Registration is
one-time.
...
3. **Verify name matching**: Ensure the `name` field in SKILL.md frontmatter matches the
   directory name. If not, edit the frontmatter:
   ```bash
   # Check current name
...
If it doesn’t, rename the directory or edit the frontmatter.
