...
The dispatched vault-maintenance subagent repairs only after a concrete recovery trigger and after inspecting the paths involved in the failing operation.
...
1. Classify each affected path:
   ...
   - unrelated local changes to preserve and exclude from recovery. They conflict with recovery only when the failure or repair must change those exact paths.
...
If the right repair is unclear, escalate the affected paths, inspected diff summary, and exact failing command to the parent. Do not treat unrelated worktree state as a reason to block normal memory work.
