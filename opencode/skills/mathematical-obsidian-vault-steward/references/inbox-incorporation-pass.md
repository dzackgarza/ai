...
- Leave sources needing more analysis in `.annotated` with local CriticMarkup explaining the unresolved passages.
...
- Run the analysis quality gate first. If the annotated source is mostly metadata, hashes, candidate target lists, broad section buckets, generic `accepted target` comments, or appended summary prose, stop before editing durable notes.
...
- If the artifact clearly has unfinished local analysis, incorporate only if the user explicitly asked for partial incorporation; otherwise leave it in `.annotated` for another analysis pass.
- Verify that claim inventories, proof-sketch reconstructions, and "remaining things to prove" sections have passage-local annotations or explicit no-import reasons at the subsection, paragraph-cluster, or mathematical-unit level. If those sections are only covered by appended summary prose, one trailing "duplicate" comment, or a bucket-level note-title map, the artifact is not incorporation-ready.
- If any text says "fully mapped", "no blockers", "no durable note edits required", or "ready to retire" while long claim-list turns are covered only by broad comments, ignore that claim and treat the artifact as needing direct analysis.
- If the only comment on a long mathematical answer appears after its references/source list, treat that as missing passage-local analysis. The incorporation agent should block or repair the analysis before editing durable notes.
- Treat claims such as "no new durable claims", "all content routes into existing notes", or "already covered" as proposals to verify, not conclusions. For each major source unit, inspect the named durable note section that allegedly absorbs or rejects it.
- "Already covered" still needs an incorporation decision: merge wording, reject as duplicate, add caveat, demote/promote, preserve source-visible, or block for human review.
...
- Verify that each routing comment uses an actual incorporation action. Non-action routes such as `cross-check`, `review`, or `compare against` are blocked unless you can repair them by reading the whole source and target notes.
- Reject comments that refer to another comment such as "see Segment C" instead of restating the actual target, action, status, and reason.
...
- If a disputed source claim already appears in a durable note, inspect that note before accepting a source-visible-only decision. The likely durable action is a local caveat, demotion, rejection, objection-resolution remark, or `needs-human` blocker near the affected statement.
...
If the analysis pass is shallow but the user explicitly asks you to repair it, switch roles and perform the missing whole-source synthesis before editing durable notes. Otherwise, leave the source in `.annotated` with local CriticMarkup at the passages that show why incorporation is not justified.
...
For partial incorporation, keep the source in `.annotated` unless unresolved passages are explicitly marked locally and the user asked to advance it.
