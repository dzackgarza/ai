...
- When many broad annotations remain, handle one coherent source segment only. Do not plan or attempt a whole-file cleanup pass. Improve one turn, section, paragraph cluster, or claim family enough that a later direct pass can continue from the artifact itself.
...
- Choose the strongest mathematically correct label:
  ...
  - recipe, quotient, disjoint union, normalization, family, package, model, or procedure to build -> `construction`
