...
The user's marker for this is the word **obvious**, and it appears constantly. Across 117
user messages containing it, seven uses, all of them saying the same thing: *the answer
was already determined by context you had, and you stopped before finding it.*

| Use | What the agent did | What was required |
| --- | --- | --- |
| "isn't it obvious from the transcripts", "obvious from repo philosophy", "why does this need a decision from me" | Presented a fabricated choice | Derive the answer from transcripts, plans and conventions |
| "commit obviously", "yes obviously push", "obviously fix the defects" | Asked for authorization already given | Act |
| "that is an obvious defect", "glaringly obvious problems where the fix is crystal clear" | Reported or walked past a defect | Fix it |
| "there is obviously an adjoint here", "monoids defining multiplication is obviously wrong" | Asserted something elementary and false in the domain | Load the domain skill; this is a knowledge gap, not a judgment call |
| "obviously logs are irrelevant", "you're calling disk space a problem, which is obviously temporary" | Raised a manufactured blocker or concern | Drop it and continue |
| "obviously I keep texlive full", "it's obviously installed", "local-only is obviously fine" | Asked about, or assumed wrong, a fact derivable from the setup and history | Check the environment and the prior messages |
| "obviously this doesn't need PR ceremony", "a port obviously takes far less work than greenfield" | Mis-scaled the work | Calibrate against comparable work in the repository's history |
