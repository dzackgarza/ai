...
| Codex waits for an external PR/CI event | The Codex PTY-loop pattern | Read the `codex` skill; it exits a one-off PTY process when new information arrives |
| Claude Code receives an external PR/CI event | Claude Code Channels | Read the `claude-code` skill; Channels provide native delivery into an active session |
...
- For an active Codex wait or PR loop, read `codex`.
- For an active Claude Code wait or PR loop, read `claude-code`.
