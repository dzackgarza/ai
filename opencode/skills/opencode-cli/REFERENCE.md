...
Comprehensive documentation for OpenCode - the open-source AI coding agent built for the
terminal.
...
It’s a Go-based TUI (Terminal User Interface) application that provides AI-powered
coding assistance directly from the command line.
...
**Project status:** The original opencode-ai/opencode repository was archived on Sep 18,
2025\. The project continues at anomalyco/opencode.
...
Download from [releases page](https://github.com/anomalyco/opencode/releases) or
...
All tools are **enabled by default** and don’t require permission.
...
Tools like `grep`, `glob`, `list` use ripgrep and respect `.gitignore`. Create a
`.ignore` file to include ignored paths:
...
**Note:** Configs are **merged**, not replaced.
