...
- Read `references/slash-command-design.md` before creating or simplifying OpenCode
  slash commands.
...
The canonical OpenCode workspace is `~/ai/opencode`, **symlinked to
`~/.config/opencode`**.
...
**Avoid using the OpenCode CLI for real LLM work.** Use `ocm` via `uvx` for chatting,
testing, and session orchestration.

## Web UI

`opencode web` runs a Web UI frontend that hooks into the `serve` backend.
This is part of OpenCode, not an external library.
