...
Generate `.excalidraw` files for architecture diagrams, flowcharts, sequence diagrams,
concept maps, and more.
Files can be opened at excalidraw.com or uploaded for shareable links.
...
- `x` is the LEFT edge.
  To center at position `cx`: `x = cx - (text.length * fontSize * 0.5) / 2`
...
`fixedPoint` coordinates: `top=[0.5,0]`, `bottom=[0.5,1]`, `left=[0,0.5]`,
`right=[1,0.5]`
...
- Emit progressively: background zones → shape → its bound text → its arrows → next
  shape
