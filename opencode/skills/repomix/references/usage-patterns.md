...
```
**Use:** New codebase, architecture review, complete LLM context, planning **Tips:**
Remove comments, use markdown, check token limits, review before sharing
...
```
**Use:** Feature analysis, debugging specific areas, targeted refactoring **Tips:**
Include related files only, stay within token limits, use XML for AI
...
```
**Use:** Feature branch review, change impact, before/after comparison, migration
planning
...
```
**Workflow:** Package library → enable security checks → review vulnerabilities → check
suspicious patterns → AI analysis **Check for:** API keys, hardcoded credentials,
network calls, obfuscation, malicious patterns
...
```
**Checklist:** No sensitive data, no test credentials, env vars correct, security
practices, no debug code
...
```
**Use:** Suspicious dependency, security advisory, license compliance, vulnerability
assessment
...
```
**Include:** Source, licenses, docs, configs.
**Exclude:** Test data, dependencies
...
```
**Use:** API docs, architecture docs, user guides, onboarding **Tips:** Include existing
docs, include source, use markdown
...
```
**Include:** Routes, controllers, schemas, middleware **Workflow:** Package → AI →
OpenAPI/Swagger → endpoint docs → examples
...
```
**Consider:** Package-specific patterns, shared deps, cross-package refs, workspace
structure
...
**Problem:** Security scanner flags secrets **Actions:** Review files → add to
`.repomixignore` → remove sensitive data → use env vars
```bash
...
**Execution:** Start broad, refine narrow → use appropriate format → enable security
checks → monitor tokens

**Review:** Verify no sensitive data → check completeness → validate format → test with
LLM
