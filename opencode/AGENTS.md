...
**Before writing any completion report or status response:** LOAD `response-preparation` skill.

Do not summarize what was done — the git commit is the summary.
Do not fill response templates mechanically. Every item in your response must pass the theory-of-mind test: "Why would the user want to read this?"
...
- **Maintain hierarchical task framing.** LOAD `hierarchical-task-framing` skill when completing sub-tasks or reporting progress.
