...
export const GitCommitPlugin: Plugin = async ({ $ }) => {
  if (!isPluginEnabled("git-commit")) return {};
  return {
    tool: {
      git_commit: tool({
        description:
        ...
        async execute(args, context) {
          ...
          const result = await $`cd ${directory} && git commit --allow-empty -m ${args.message}`.nothrow();
          ...
          return result.stdout?.toString().trim() || "[git_commit] Commit successful";
        },
      }),
    },
  };
