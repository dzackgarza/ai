...
Historical note: these findings came from the pre-extraction local harness snapshot.
The active command surface now lives in `dzackgarza/opencode-manager`, while this
directory remains a compatibility layer during retirement.

### Current `plan_exit` behavior

- `plan_exit` creates a new orchestrator session and primes it with the Orchestrator
  agent prompt.

- It returns the orchestrator `Session ID` and `Title` so the user can switch sessions
  manually.
...
- Use raw server API only when debugging protocol-level behavior or integrating from
  non-JS runtimes.
