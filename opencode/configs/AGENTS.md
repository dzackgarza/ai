...
This file is **auto-generated**. Any manual edits will be overwritten the next time `build_config.py` runs. Treat it as a build artifact, not source code.
...
These files get their `permission` fields regenerated by `manage_permissions.py --apply`. Your edits will be overwritten. **Edit `manage_permissions.py` instead** to change permissions.
