...
You are tasked with implementing an approved technical plan retrieved from memory using
`list_memories`. These plans contain phases with specific changes and success criteria.
...
validation step is separate and should have created a memory record with status
VALIDATED.

**Check for validation status:** Use the `list_memories` tool to check for plan
validation records.
