...
Every project root contains a `.agents/` directory. This is the canonical location for agent-facing project artifacts that are not durable memory:
...
Durable operational knowledge and planning state are managed through `agent-memory` and bound by `.agent-memory.toml`; they are not maintained as loose markdown under `.agents/`.
