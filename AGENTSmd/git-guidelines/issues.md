...
- `uvx --from git+https://github.com/dzackgarza/itree itree new OWNER/REPO "Title" --under OWNER/REPO#PARENT` to file a new work unit under a grouping issue (omit `--under` to receive non-mutating placement guidance).
...
For governed milestone-and-ledger creation, use the canonical released route in
`opencode/skills/git-guidelines/SKILL.md`; do not replace it with a manual governed
sequence.
