...
An SDF must satisfy the **Lipschitz condition**: `|f(a) - f(b)| ≤ |a - b|` (gradient
magnitude ≤ 1). This guarantees that stepping by the SDF value is always safe — no
surface exists within that radius.
...
2. **Distance fade**: `d += amp * fbm(p * freq) * smoothstep(fadeStart, 0.0, d)` —
   detail only appears near the surface where overshoot distance is small
...
6 iterations is the practical sweet spot — gives sub-pixel precision without wasting GPU
cycles.
...
This is equivalent to: `union(a, b) AND NOT intersection(a, b)` — the symmetric
difference. Geometry exists where exactly one shape is present but not both.
Useful for creating lattice structures and interlocking patterns.

## Interior SDF Pattern Techniques

When the camera is inside an SDF (d < 0), the negative distance still gives useful
information:
