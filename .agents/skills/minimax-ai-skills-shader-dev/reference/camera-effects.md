...
A real camera lens focuses light from a focal plane onto the sensor.
Points not on the focal plane project to a **circle of confusion (CoC)** on the sensor.
...
Advantages over regular grid: no structured aliasing patterns, better coverage per
sample count.
...
```
Use `t_sample` for all scene animation.
Accumulate multiple frames for convergence.
...
Use temporal accumulation (TAA-style) with per-frame time jitter — converges over frames
with no per-frame cost increase.
...
- **Spatially uncorrelated**: Use pixel coordinates in hash, not UV (grain should be
  screen-resolution)
