...
def get_opencode_sessions():
    ...
    # Primary: query Langfuse for full session IDs (log filenames are truncated)
    try:
        from langfuse import get_client

        lf = get_client()
        for s in lf.api.sessions.list(limit=50).data:
    ...
    # Fallback: log filenames (truncated IDs — may not work with Langfuse API)
    if not sessions:
...
def get_amp_sessions():
    ...
    try:
        result = subprocess.run(
            ["amp", "threads", "list"], capture_output=True, text=True
        )
        if result.returncode != 0:
            return sessions
...
def main():
    ...
    print(
        f"{'HARNESS':<15} | {'PROJECT':<30} | {'LAST MODIFIED':<20} | {'IDENTIFIER / FILE'}"
    )
    print("-" * 130)
