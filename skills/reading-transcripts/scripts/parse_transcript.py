...
def main():
    parser = argparse.ArgumentParser(description="Unified transcript parser for all agent CLIs")
    ...
    try:
        if args.harness == "opencode":
            # OpenCode requires piping the export
            cmd = f"opencode export {args.identifier} | python {scripts_dir}/parse_opencode_log.py -"
            ...
            if args.identifier == "-":
                # Handle piped stdin
                subprocess.run(["python", script_map[args.harness], "-"], stdin=sys.stdin, check=True)
            else:
                subprocess.run(["cat", args.identifier], stdout=subprocess.PIPE, check=True)
                subprocess.run(f"cat {args.identifier} | python {script_map[args.harness]} -", shell=True, check=True)
