...
**CRITICAL: The next message AFTER a compaction block IS the compaction summary.**

**What compaction looks like in transcripts:**

```
[ASSISTANT]
<compaction>  ← This is JUST a marker. It contains ZERO content.

[ASSISTANT]  ← THE COMPACTIFICATION ASSISTANT
## Summary of Previous Conversation
- Key decision: We chose approach X because...
- Open issue: Need to verify Y...
- Next step: Implement Z...

↑↑↑ THIS MESSAGE IS THE COMPACTION SUMMARY ↑↑↑
```

**The compaction marker is NOT a summary.** It is literally just a timestamp marking WHEN compaction occurred. **The VERY NEXT assistant message IS the compaction summary**, generated by the compactification assistant.

**Why this matters:**

| What you see | What it means |
|--------------|---------------|
| `<compaction>` marker | "Compaction happened here" — contains NO information |
| Next assistant message | **THE COMPACTION SUMMARY** — contains ALL preserved context |

**Rule:** When you see a compaction marker, **THE NEXT MESSAGE IS THE SUMMARY**. Do not skip it. Do not scroll past it. That message IS the compactification assistant's preserved record of everything important from before the compaction event.

**The compactification assistant's job:** After marking the compaction point, it IMMEDIATELY writes the summary as the next message. That summary is the ONLY record of what happened before compaction.
